...
if SAM_API_KEY:
    # Fetch real data with better filters
    try:
        ...
        params = {
            ...
            'postedTo': '01/28/2026',
            'noticeType': 'o,p,k',  # Opportunities, Pre-solicitations, Combined
            'limit': 10  # Get 10, pick the best one
        }
        ...
        if opportunities:
            # Find the one with most text (likely has contacts)
            opp = max(opportunities, key=lambda x: len(x.get('description', '')))
            desc_len = len(opp.get('description', ''))
            print(f"   ✓ Fetched: {opp.get('title', 'Untitled')[:60]}...")
            print(f"   ✓ Description length: {desc_len} characters")
            
            if desc_len < 200:
                print(f"   ⚠️  Very short description, trying point of contact...")
                # Try to get POC info
                poc = opp.get('pointOfContact', [])
                if poc and len(poc) > 0:
                    print(f"   ✓ Found point of contact!")
        else:
