...
@app.route('/network')
def network_view():
    """Serve network visualization page"""
...
# Network and Contacts API Routes
...
def get_network_data():
    ...
    try:
        ...
        network_file = Path("data/network/contact_network.json")
        ...
        if network_file.exists():
            ...
            # Return empty network if file doesn't exist
...
def get_network_stats():
    ...
    try:
        ...
        # Total agencies
        c.execute("SELECT COUNT(DISTINCT agency) FROM contacts")
        result = c.fetchone()
        total_agencies = result[0] if result else 0
        
        # Decision makers
        c.execute("SELECT COUNT(*) FROM contacts WHERE role_type = 'Decision Maker'")
        result = c.fetchone()
        decision_makers = result[0] if result else 0
        
        # Recent interactions (last 30 days)
        c.execute("SELECT COUNT(*) FROM interactions WHERE interaction_date >= date('now', '-30 days')")
        result = c.fetchone()
        recent_interactions = result[0] if result else 0
    ...
    try:
        ...
        c.execute("""
            SELECT id, first_name, last_name, title, organization, department,
                   email, phone, agency, role_type, influence_level
            FROM contacts
        ...
        for row in c.fetchall():
            ...
            c.execute("SELECT tag FROM contact_tags WHERE contact_id = ?", (row[0],))
...
def get_contact_detail(contact_id):
    ...
    try:
        ...
        contact = {
            ...
            'linkedin_url': row[8],
            'agency': row[9],
            'office_symbol': row[10],
            'location': row[11],
            'clearance_level': row[12],
            'role_type': row[13],
            'influence_level': row[14],
            'notes': row[15]
        }
        
        # Get tags
        c.execute("SELECT tag FROM contact_tags WHERE contact_id = ?", (contact_id,))
        contact['tags'] = [tag[0] for tag in c.fetchall()]
        
        # Get relationships
        c.execute("""
            SELECT cr.id, cr.contact_id_2, c.first_name, c.last_name, 
                   c.title, c.organization, cr.relationship_type, cr.strength, cr.notes
            FROM contact_relationships cr
            JOIN contacts c ON cr.contact_id_2 = c.id
            WHERE cr.contact_id_1 = ?
        """, (contact_id,))
        
        contact['relationships'] = []
        for rel_row in c.fetchall():
            contact['relationships'].append({
                'id': rel_row[0],
                'contact_id': rel_row[1],
                'name': f"{rel_row[2]} {rel_row[3]}",
                'title': rel_row[4],
                'organization': rel_row[5],
                'relationship_type': rel_row[6],
                'strength': rel_row[7],
                'notes': rel_row[8]
            })
        ...
        # Get interactions
        c.execute("""
            SELECT id, interaction_type, interaction_date, subject, summary,
                   outcome, next_action, next_action_date, our_team_member
            FROM interactions
            WHERE contact_id = ?
        ...
        for int_row in c.fetchall():
            contact['interactions'].append({
                ...
                'summary': int_row[4],
                'outcome': int_row[5],
                'next_action': int_row[6],
                'next_action_date': int_row[7],
                'our_team_member': int_row[8]
            })
        ...
        print(f"Error loading contact detail: {e}")
        return jsonify({'error': str(e)}), 500

"""
MINIMAL CONTACT MANAGEMENT PATCH
Add these endpoints to team_dashboard_app.py right after the existing /api/contacts routes
"""
...
def create_contact():
    ...
    try:
        ...
        """, (
            ...
            data.get('organization'), data.get('department'), data.get('email'),
            data.get('phone'), data.get('linkedin_url'), data.get('agency'),
            data.get('office_symbol'), data.get('location'), data.get('clearance_level'),
            data.get('role_type'), data.get('influence_level'), data.get('notes')
        ))
        ...
        if data.get('tags'):
            for tag in data['tags']:
                if tag.strip():
                    c.execute("INSERT INTO contact_tags (contact_id, tag) VALUES (?, ?)",
        ...
        for first_name, last_name, count in name_duplicates:
            ...
            for row in c.fetchall():
                contacts.append({
                    ...
                    'title': row[3], 'organization': row[4], 'email': row[5], 'phone': row[6]
                })
            
            duplicates.append({'type': 'name', 'contacts': contacts})
...
def preview_import():
    ...
    try:
        ...
        return jsonify({'preview': preview, 'total': len(df), 'columns': list(df.columns)})
    ...
    try:
        ...
        for idx, row in df.iterrows():
            try:
                ...
                """, (
                    str(row.get('first_name', '')), str(row.get('last_name', '')),
                    ...
                    str(row.get('agency', '')) if pd.notna(row.get('agency')) else None,
