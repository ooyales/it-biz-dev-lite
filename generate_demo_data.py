...
def create_dummy_opportunity(index, spread_timeline=True):
    """Create a single realistic opportunity"""
    
    # Random dates - spread across 2026 and early 2027
    if spread_timeline:
        # Spread deadlines from now through end of 2027
        days_from_now = random.randint(30, 730)  # 30 days to 2 years
        deadline = datetime.now() + timedelta(days=days_from_now)
        posted_date = deadline - timedelta(days=random.randint(30, 90))
    else:
        # Original behavior - shorter timeline
...
def create_database_data(num_opportunities=30):
    ...
    for i in range(1, num_opportunities + 1):
        opp = create_dummy_opportunity(i, spread_timeline=True)
