...
def get_network_stats():
    ...
    try:
        ...
        total_contacts = c.fetchone()[0] if c.fetchone() else 0
        
        # Total relationships
        c.execute("SELECT COUNT(*) FROM contact_relationships")
        total_relationships = c.fetchone()[0] if c.fetchone() else 0
        
        # Total agencies
        c.execute("SELECT COUNT(DISTINCT agency) FROM contacts")
        total_agencies = c.fetchone()[0] if c.fetchone() else 0
        
        # Decision makers
        c.execute("SELECT COUNT(*) FROM contacts WHERE role_type = 'Decision Maker'")
        decision_makers = c.fetchone()[0] if c.fetchone() else 0
